...
5. Sign `Memento.app` and the DMG with the Developer ID identity from `.env`.
...
8. Bundle `Memento.app.tar.gz` and produce `Memento.app.tar.gz.sig` using the Tauri updater key.
9. Write `latest.json` with the new version, signature, and download URL pointing at `GithubAnant/memento`.
10. Create a **draft** release on `GithubAnant/memento` via `gh release create --draft`, uploading the DMG, the updater tarball, and `latest.json`, with the drafted notes attached.
...
- Three assets are attached: `Memento_<version>_aarch64.dmg`, `Memento.app.tar.gz`, `latest.json`.
...
- Confirm `https://github.com/GithubAnant/memento/releases/latest/download/latest.json` resolves to the new version. The in-app updater hits this URL on launch (see `apps/desktop/src-tauri/tauri.conf.json`).
...
- **`gh release create` fails because the tag already exists in `memento`.** A previous attempt got far enough to publish. Do not delete the existing release without explicit user permission — it may already be live to users via the updater. Ask first.
