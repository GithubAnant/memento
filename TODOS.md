...
- GitHub Source Control UI: [`SPECs/github-sync-spec.md`](SPECs/github-sync-spec.md) — prominent top-right sync button (live change count, menu with Push/Fetch/View changes/Sign out), a VSCode-style Source Control tab (per-file change list + read-only CodeMirror merge diff + per-file discard + commit-message box & Push), and a Stats tab with a commit-activity heatmap. New backend commands `github_changed_files`, `github_file_diff`, `github_discard_file`, `github_commit_history`. Added File/View/GitHub/Go native menus and set `mainBinaryName` so the dev binary (and Dock) reads "Memento".
- GitHub sync (auth + clone + pull/push): [`SPECs/github-sync-spec.md`](SPECs/github-sync-spec.md) — Personal Access Token auth (token validated against `GET /user`, then stored in the macOS Keychain via `keyring`), clone the private memory repo to `~/Desktop/Memento/<repo>/` + write a `~/.memento/state.json` pointer for agent discovery, auto-fetch (interval + focus + visibility, fast-forward only over a clean tree), and a subtle status-bar sync control that commits all dirty files + pushes with fetch-then-retry. Embedded `git2` transport (no system git); added the `com.apple.security.network.client` entitlement. Single-author/agent-primary model → no merge UI; non-fast-forward divergence surfaces an explicit push-or-discard banner. PAT auth needs no GitHub OAuth app registration — the user pastes a token with the `repo` scope.
