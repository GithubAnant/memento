...
- GitHub sync (auth + clone + pull/push): [`SPECs/github-sync-spec.md`](SPECs/github-sync-spec.md) — OAuth device-flow auth (token in macOS Keychain via `keyring`), clone the private memory repo to `~/Desktop/Memento/<repo>/` + write a `~/.memento/state.json` pointer for agent discovery, auto-fetch (interval + focus + visibility, fast-forward only over a clean tree), and a subtle status-bar sync control that commits all dirty files + pushes with fetch-then-retry. Embedded `git2` transport (no system git); added the `com.apple.security.network.client` entitlement. Single-author/agent-primary model → no merge UI; non-fast-forward divergence surfaces an explicit push-or-discard banner. **Before live use:** register the GitHub OAuth app (enable Device Flow) and inject its `GITHUB_CLIENT_ID` env at build time.
