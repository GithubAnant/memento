...
End-to-end tests for the Memento desktop app, driven via WebdriverIO and the
...
> The e2e build uses an isolated bundle identifier (`com.memento.e2e`)
> so it does NOT collide with `tauri-plugin-single-instance` from a Memento
> dev or release instance running in another worktree. You can leave your
> normal Memento running.
...
1. `pnpm run build:app` produces `Memento.app` with the `e2e` Cargo feature,
...
2. `wdio.conf.js` (`onPrepare`) spawns the `tauri-webdriver` intermediary CLI
   on port 4444.
3. WebdriverIO connects to 4444; the intermediary launches `Memento.app` and
...
- The e2e build invokes
  `cargo tauri build --features e2e --bundles app --config '{"identifier":"com.memento.e2e","bundle":{"createUpdaterArtifacts":false}}'`.
  The overrides:
  ...
  - `identifier: "com.memento.e2e"` gives the e2e build its own
    `tauri-plugin-single-instance` namespace and its own app data dir
    (`~/Library/Application Support/com.memento.e2e/`). Without this,
    a Memento dev/release instance running in another worktree would intercept
